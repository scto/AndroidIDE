...
enum class ParameterConstraint {
  ...
  /**
   * Used with [FILE] and [DIRECTORY]. Asserts that the file/directory at the
   * given path exists.
   */
  EXISTS,
}
...
abstract class Parameter<T>(
  ...
  /**
   * Resets the parameter value to the default value and removes any external
   * value observers.
   *
   * @param notify Whether the observers should be notified about this change or
   *   not.
   */
 ...
 *
 * @property startIcon The drawable resource that will be used as the start
 *   icon.
 * @property endIcon The drawable resource that will be used as the end icon.
 * @property onStartIconClick Function which will be used when the start icon is
