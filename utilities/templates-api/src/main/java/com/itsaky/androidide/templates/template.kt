 ...
 *
 * A [TemplateRecipeConfigurator] is called just before the [TemplateRecipe] is
 * executed.
 */
 ...
 *
 * A [TemplateRecipeFinalizer] is called just after the [TemplateRecipe] is
 * executed.
 */
...
data class ModuleVersionData(
  ...
  /**
   * Get the Java source version string representation in the
   * `JavaVersion.VERSION_${version}` format.
   */
  fun javaSource() = javaVersionPrefix(javaSource)

  /**
   * Get the Java target version string representation in the
   * `JavaVersion.VERSION_${version}` format.
   */
...
open class Template<R : TemplateRecipeResult>(
  ...
  companion object {
    ...
    @JvmStatic val EMPTY = Template(-1, -1, emptyList(), EMPTY_RECIPE)
  }
}
  ...
  override val widgets: List<Widget<*>>
    get() =
      if (moduleTemplates.isEmpty()) super.widgets
        ...
        super.widgets.toMutableList().apply {
...
) : Template<R>(name, thumb, widgets, recipe)
 ...
 * @property widgets The widgets that will be rendered while creating this
 *   template.
